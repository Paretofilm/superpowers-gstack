...
Fase 4 er ferdig når tabellen står i specen med lik dom i 6/6, Step 9 er `CLEAN`, og `bash tests/run.sh --unit` viser **457 passed** lokalt (449 + zsh-regresjonstesten fra Step 3 + alarmtesten + 6 fra Step 9 sine Codex-funn; i CI 456 passed + 1 skipped). Landing er neste beslutning, ikke en del av denne fasen: `/ship` — den fulle pipelinen — kjører Step 8 en gang til på veien, som et sjuende datapunkt — forutsatt at planen ligger der Step 8 søker (`~/.gstack/projects/<slug>/`, `~/.claude/plans`, `~/.codex/plans`, `.gstack/plans`) eller i samtalekonteksten; `docs/superpowers/plans/` er ikke en søkesti, så legg en kopi i `~/.gstack/projects/Paretofilm-superpowers-gstack/` før landing og fjern den etterpå.
...
**Testtall** (CI-kommandoen `pytest tests/unit scripts/cost-ledger -q`): 378 → 388 (fase 1 som planlagt, +10) → 404 (review-rettelsene på fase 1, +16) → 419 (fase 2 som planlagt, +15) → 433 (review-rettelsene på fase 2, +14) → 442 (fase 3 som planlagt, +9) → 449 (review-rettelsene på fase 3, +7) → 451 lokalt / 450 + 1 skipped i CI (fase 4, +2: zsh-regresjonstesten fra Step 3 og alarmtesten) → 457 lokalt / 456 + 1 skipped i CI (Step 9, +6: fem `summary`-typer i `verdict`, én `\r` i repin-diffen).
