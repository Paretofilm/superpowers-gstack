...
### M2. ✅ ACCEPTED-DEFERRED (by design) — Commit steps not idempotent

**Status: deferred-by-design.** The backlog finding itself notes the acceptable mitigation: "the executing agent (or human) handles this by reading current git state before committing." Real fix (dynamic commit-message generation based on staged diff) is explicitly out of scope per the original codex flag. This pattern is documented and accepted; the plan file remains as a historical artifact.
...
## Triage summary (final — all items resolved 2026-05-18)

| Finding | Status | Shipped in |
|---|---|---|
| S1 (real dispatch tests) | ✅ FIXED | v2.6.0 (integration test infrastructure) |
| S2 (ios-native-review) | ✅ FIXED | v2.4.0 |
| S3 + S4 (schema + convergence) | ✅ FIXED | v2.5.0 (together) |
| S5 (locale JSON bug) | ✅ FIXED | v2.2.0 (caught at implementation time, folded into release) |
| S6 (versioned routing) | ✅ FIXED | v2.3.2 |
| S7 (office-hours next-step) | ✅ FIXED | v2.3.0 (office-hours-track-aware wrapper) |
| M1 (contrast-check doc) | ✅ FIXED | v2.8.1 |
| M2 (commit idempotency) | ✅ ACCEPTED-DEFERRED | by-design — executing agent handles git state |
| M3 (Phase 2/3 parallelism) | ✅ FIXED | v2.8.1 |

**v1.1 backlog status: CLOSED.** All SIGNIFICANT findings shipped. M1 + M3 cleanup landed in v2.8.1. M2 is accepted-deferred-by-design (the codex flag itself documents the acceptable mitigation).

**Follow-up work generated by v2.7-v2.9 patches** is tracked separately in `docs/superpowers/backlog/2026-05-18-followups.md` (post-shipment deferred items: additional integration tests, CI integration, non-Apple companion-skill entries).
