...
- `mode:` takes `manual` or `continuous`. Two legacy spellings mean `continuous` and MUST still be honoured on read: the YAML value `auto` (pre-2.36.0) and the `## Mode: auto` Markdown marker (pre-2.1.1). Precedence on read: YAML value first, Markdown marker second. On the next write, always emit `mode: continuous` and delete the Markdown marker — never write `auto` again.
