...
- **You must rotate the simulator yourself before the run.** Neither `simctl` nor `idb` can rotate a simulator programmatically, so the tool does **not** set orientation — it only *verifies* it. Rotate in Simulator.app (Cmd+← / Cmd+→) so the app is in the orientation you pass to `--orientation`, then launch. If the Application frame's aspect doesn't match, preflight fails closed with a "rotate the sim first" message rather than running against the wrong layout.
