...
# Exit codes: 0 = tests passed, 2 = setup error (no simulator runtime), any other
# non-zero = a failed test run. That status is xcodebuild's own, passed through
# unchanged, so it is typically 65 — NOT 1. Check `-ne 0`, never `-eq 1`.
...
# Deliberately NOT normalised to 1: xcodebuild's own code distinguishes a test failure
# (65) from a build failure (also non-zero, different code), and flattening them would
# throw that away. Callers must test for non-zero.
exit "$TEST_STATUS"
```
