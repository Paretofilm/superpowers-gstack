...
**Capability workflow (e.g. CloudKit):** (1) agent declares the capability + container in `*.entitlements` (declarative XML); (2) agent ensures `DEVELOPMENT_TEAM` is set in `project.yml`; (3) agent runs `xcodegen generate` + `xcodebuild build`; (4) if signing fails with "container not registered" / "no matching provisioning profile" — STOP and hand the user the exact portal steps (developer.apple.com/account → Identifiers → App ID → enable the capability → link the container; ~30 sec), then retry the build after their click. The portal requires Apple ID + 2FA that no MCP/CLI tool automates — the declarative entitlements + `project.yml` path is the only one that works for both the agent and git history.
