...
- Ship-worthy / architecture / real-time / security / contract change, after Claude self-pitfall + Codex have reviewed → invoke /superpowers-gstack:third-lens-review. Runs a third (and optionally fourth) external model house via OpenRouter on the PATCHED artifact (`scripts/third-lens-review.py`), then an adversarial synthesis. Routing by `--role`: architecture=GLM-5.2 (default), sensitive=Gemini 3.1 Pro (Western infra, enforced by `--sensitive`), correctness=DeepSeek V4-Pro, countersynthesis=GPT-5.5. Gated by stakes — skip for trivial/standard changes. ~$0.05/run; key in Keychain `openrouter-api-key`.
