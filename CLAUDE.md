...
After /compact: check whether continuous handoff is already active, in this priority order — YAML `mode: continuous`, then the legacy YAML value `mode: auto` (pre-2.36.0), then the legacy `## Mode: auto` Markdown marker (pre-2.1.1). If none is present, ask the user once: "Context was compressed. Want me to keep handoff.md updated continuously for this session? I'll refresh it at each milestone and suggest /clear when context gets heavy." If yes, invoke the context-handoff skill. This is a handoff-persistence setting and has nothing to do with Claude Code's **auto mode**, which is a permission mode — never change one because of the other.
