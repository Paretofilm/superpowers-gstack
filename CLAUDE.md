...
On session start or after /compact, look at `docs/superpowers/handoff.md`. Consume it ONLY on positive identification — reading a handoff clears it, so misidentifying the file destroys its contents. `type: handoff` in frontmatter is the current format (v2.1.1+); YAML carrying both `session_end` and `next_step` without `type:` is a legacy handoff (v1.12.0 → v2.1.0). Both are consumed. **Anything else, including a file with no frontmatter at all, is not a handoff**: say in one line that the file exists and holds unrecognized content, do not present it as where you left off, and do not clear it (pre-1.12.0 prose-only auto-consume was removed in 2.36.1 — it silently destroyed project notes kept at that path). When it is a handoff, quote `next_step` verbatim, name the `active_task` ID, and surface `env` (venv, dev_server, test_cmd) so commands work immediately. Then proceed normally — do not ask "ready to continue?". Read the `mode:` field BEFORE clearing — clearing first destroys the value the next paragraph needs. If the handoff was in continuous mode — that is, `mode: continuous` **or** the legacy `mode: auto` — do NOT blank the file: rewrite it carrying just `type: handoff` and `mode: continuous`, so the setting survives the next compact. Otherwise clear it (write empty string) immediately after presenting the summary.
