...
def test_route_checks_that_an_existing_runner_honours_the_axis():
    """2.53.0 changed the TEMPLATE; it did not touch runners already written into
    projects. A pre-2.53.0 runner never reads the pin, so `executor=vm` runs on the host
    and says nothing — arriving through the very entry point that claims to cover both
    executors. Reported by the rig session, which hit it in swiftconfig."""
    assert r"grep -qE '\.gstack/e2e-executor' scripts/run-uitests.sh" in ROUTE
    assert "LEGACY" in ROUTE and "predates 2.53.0" in flat(ROUTE)
