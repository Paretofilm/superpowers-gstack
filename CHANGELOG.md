...
Main took 2.51.1 for a verified upstream sync while this fix was in flight, so it
ships as 2.51.2. 2.51.0 had merged at the commit before it; the defect is in the
lens script, not the gate.
...
- **`mode: auto` → `mode: continuous`** in `docs/superpowers/handoff.md`. Claude
  ...
  Reading is backwards compatible: YAML `mode: continuous`, then the legacy YAML
  `mode: auto` (pre-2.36.0); the `## Mode: auto` Markdown marker (pre-2.1.1)
  counts only when the YAML key is absent entirely. Writes always emit
  `continuous`. Existing handoff files keep working and are rewritten on their
  next update — **but the rename alone is not the whole release**: the sensor fix
  below reaches a project only when its CLAUDE.md is regenerated by
  `/superpowers-gstack:adapt`. The shipped SessionStart hook nudges for that
  automatically once the version marker falls behind, so no one has to remember;
  it is still a step someone has to take.
  Forward compatibility has one gap worth naming: a handoff written as
  `mode: continuous` and then opened by a **pre-2.36.0** CLAUDE.md is blanked
  unconditionally, because that older instruction had no preserve-on-continuous
  branch. The setting is lost until the next 2.36.0 session rewrites it. It
  self-heals, and the old sensor could not read `continuous` anyway, so nothing
  regresses — but mixed-version teams should upgrade together rather than
...
- `tests/unit/test_lint_heading_text.py` pins `heading_text()` in both
  directions: six re-paste disguises that must be caught, and six non-heading
  lines that must not be. The false-positive half matters as much as the other —
  a lint that blocks a legitimate release stops being trusted faster than one
  that misses a duplication. Also asserts every `MARKER_BLOCKS` entry still
  exposes a parseable heading, since E8 builds its comparison set from that call
  and would go quiet, not loud, if a block's first line stopped parsing.
