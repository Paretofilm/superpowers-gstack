...
- **Claude Code Plugin** with eight skills:
  ...
  - `/htmlify` — Generates beautiful, pedagogical HTML companions for MD artefakter (design docs, plans, handoffs). Auto-opens via `--open` flag (macOS); aggregated per-directory dashboard via `htmlify dashboard <dir>`. Optional PostToolUse hook auto-fires on artifact writes. YAML-frontmatter as schema contract (rides on `/context-handoff` v1.12.0 pattern). Bun + TypeScript implementation; 74 unit tests; sanitized via DOMPurify.
