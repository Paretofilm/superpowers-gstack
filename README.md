...
- **Claude Code Plugin** with twelve skills:
  ...
  - `/autoimplement` — Auto-advance through a multi-phase implementation plan by chaining `/review`, `/pitfall-verification`, and `/codex review` at each phase boundary. Skips the y/n prompts you'd always answer "yes" to when reviews pass; stops by default when /review or any review flags actionable issues; severe findings (security/data-loss/correctness) always stop regardless of policy; pitfall/codex findings can be treated as advisory via the policy question. Severe findings (security, data loss, correctness in test assertions) always stop regardless of policy. Hard refusals: fewer than 2 phases, missing per-phase commit steps, dirty working tree, on `main`/`master` branch, plan touches migrations / secrets / credentials / `.env` / `.ssh`.
