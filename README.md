...
- **Claude Code Plugin** with twelve skills:
  ...
  - `/third-lens-review` — Third (and optionally fourth) external model house run after Claude self-pitfall + Codex on a **ship-worthy / architecture / real-time / security / contract** change. A different training distribution sees what two Western houses both took for granted (dead code, use-after-free, degraded-state bugs). Runs via OpenRouter (`scripts/third-lens-review.py`) on the patched artifact, then a mandatory **adversarial synthesis** (finding is real until refuted). Routing by `--role`: `architecture`=GLM-5.2 (default), `sensitive`=Gemini 3.1 Pro (Western infra, enforced by `--sensitive`), `correctness`=DeepSeek V4-Pro, `countersynthesis`=GPT-5.5. Gated by stakes — skipped for trivial/standard changes. ~$0.05/run; key in macOS Keychain `openrouter-api-key`.
